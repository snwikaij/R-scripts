...
x1          <- seq(4,10,0.1)
x2          <- seq(0,8,0.1)
y1          <- exp(.1*x1+1)+rpois(length(x1),2)
y2          <- exp(0.15*x2)+rpois(length(x2),2)
...
initstart         <- function(){list(b0=runif(1), t0=runif(1), b1=runif(1), t1=runif(1))}
save_params       <- c("beta0", "beta1")
