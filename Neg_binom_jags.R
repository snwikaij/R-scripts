...
nb.mod_log     <- function(){
  ...
  for (i in 1:N){
    ...
    y[i]           ~ dnegbin(p[i], r)#likelihood of neg.binom distribution
    
    p[i]           <- r/(r+lambda[i])#accuracy of neg.binom distribution
    
    log(lambda[i]) <- mu[i]#log link
...
for(i in 1:100){
  lines(df$x, exp(as.numeric(jags_nb.mod$BUGSoutput$sims.list$b0[i])+as.numeric(jags_nb.mod$BUGSoutput$sims.list$b1[i])*df$x), alpha=0.1, col=alpha(rgb(0,0,0), 0.1))
}
